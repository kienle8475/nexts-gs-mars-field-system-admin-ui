...
import { exportSaleExcel } from "@/services/export/sale.export";
...
const SalesSection = () => {
  ...
  const { data: reportItems = [], isLoading: reportItemsLoading } = useReportItems("SALES");
  ...
  const skuGroupedColumns = Object.entries(brandGroups).map(([brand, items]) => ({
    ...
    children: items.map((item) => ({
      ...
      render: (_: any, record: any) => {
        const saleData = record.attendances?.[0]?.saleReport?.data || [];
        const found = saleData.find((d: any) => d.sku === item.skuCode);
      ...
      <div className="p-6">
        <Spin spinning={isFetching || reportItemsLoading}>
          <Table
            ...
            rowClassName={(record) =>
              record.attendances?.[0]?.saleReport?.data?.length > 0 ? "bg-green-50" : "bg-red-50"
            }
            pagination={{
              current: page,
              pageSize: pageSize,
